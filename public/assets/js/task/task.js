(() => {
  ...
  isShow ||
    $(function () {
      $("#no-record-info-msg").hide(),
        ...
        $("#project_filter,#filter_user,#filter_task").select2({
          width: "100%",
          sorter: function (e) {
        ...
        $("#filter_per_page").select2({ width: "100%" }),
        setTimeout(function () {
          $("#filter_user").select2({ width: "100%" }).trigger("change");
        }, 100),
        ...
        $("#priority,#editPriority").select2({ width: "100%" }),
              ...
              createTag: function (e) {
                ...
                if (
                  ($("#tagIds option").each(function () {
                    $.trim(e.term).toUpperCase() ===
                      $.trim($(this).text()).toUpperCase() && (t = !0);
                  }),
                  !t)
                )
        ...
        $("#dueDate,#editDueDate").datetimepicker({
          format: "YYYY-MM-DD",
          useCurrent: !1,
          locale: "ar" == languageName ? "en" : languageName,
          icons: {
        ...
        s.ajax.reload(),
        $("input.complete-task-checkbox").prop("disabled", !0),
        $(document).ajaxComplete(function (e) {
          $("input.complete-task-checkbox").prop("disabled", !1);
        }),
        $(window)
          .resize(function () {
            matchWindowScreenPixels(
    ...
    $(document).ready(function () {
      scrollToTheBottom(),
        $(".nav-tabs a").on("show.bs.tab", function () {
          1 === $(this).data("for-comment") && scrollToTheBottom();
        });
    }),
    (window.scrollToTheBottom = function () {
      setTimeout(function () {
        var e = $("#itemsWrapper").outerHeight();
        $(".task-chat-content").scrollTop(e * e);
      }, 200);
    });
  var e,
    t,
  ...
  if (!isShow)
    var s = $("#task_table").DataTable({
      ...
      ajax: {
        ...
        data: function (e) {
          (e.filter_project = $("#project_filter")
            .find("option:selected")
            .val()),
            (e.filter_user = $("#filter_user").find("option:selected").val()),
            (e.filter_status = $("#filter_status")
              .find("option:selected")
              .val()),
            (e.due_date_filter = $("#dueDateFilter").val());
        },
      },
      columnDefs: [
        { targets: [7], orderable: !1, width: "9%" },
        { targets: [0], width: "2%", className: "text-center", orderable: !1 },
        { targets: [2], orderable: !1 },
        { targets: [3], width: "6%" },
        { targets: [4, 5], width: "10%", className: "text-center" },
      ...
      columns: [
        ...
        },
        {
          data: function (e) {
            return (
              '<a href="' +
              (taskUrl + e.project.prefix + "-" + e.task_number) +
              '">' +
              e.title +
              "</a>"
            );
        ...
        },
        {
          data: function (e) {
            var t = "";
            return (
              $(e.task_assignee).each(function (e, a) {
                t +=
                  '<img class="assignee__avatar" src="' +
                  a.img_avatar +
        ...
        },
        {
          data: function (e) {
            var t = e.priority;
            return (
              '<i class="fa fa-arrow-up" style="color: ' +
              {
                highest: "#FF0000",
                high: "#FF3333",
                medium: "#FF8000",
        ...
        },
        {
          ...
          render: function (e) {
            if (null == e.due_date || "" === e.due_date) return "";
            var t = new Date().toISOString().split("T")[0];
            return 0 === e.status && t > e.due_date
              ? '<span class="text-danger">' + format(e.due_date) + "</span>"
              : format(e.due_date);
          },
          name: "due_date",
        },
        {
          ...
          render: function (e) {
            return (
              ...
              format(e.created_at, "hh:mm:ss a") +
              '">' +
              format(e.created_at) +
              "</span>"
            );
        ...
        {
          data: function (e) {
            return e.created_user
              ? '<img class="assignee__avatar" src="' +
                  e.created_user.img_avatar +
                  '" data-toggle="tooltip" title="' +
                  e.created_user.name +
                  '">'
              : "";
        ...
        },
        {
          data: function (e) {
            var t = [];
            $.each(e.task_assignee, function (e, a) {
              t.push(a.id);
            });
            var a =
              ...
              e.id +
              '"> <i class="fa fa-clock action-icon"></i></a><a title="Edit" class="btn action-btn btn-primary btn-sm mr-1 task-edit-btn" data-id="' +
              e.id +
              '"><i class="cui-pencil action-icon"></i></a><a title="Delete" class="btn action-btn btn-danger btn-sm btn-task-delete" data-task-id="' +
              e.id +
              '"><i class="cui-trash action-icon"></i></a>';
            return (
              $.inArray(loggedInUserId, t) > -1 &&
                (a +=
                  '<a title="Add Time Entry" class="btn btn-success action-btn btn-sm entry-model ml-1" data-toggle="modal" data-target="#timeEntryAddModal" data-id="' +
                  e.id +
                  '" data-project-id="' +
                  e.project.id +
  ...
  function d(e, t) {
    (o = []),
      ...
      $("#" + t).trigger("change"),
      $.ajax({
        url: route("users-of-projects") + "?projectIds=" + e,
        type: "GET",
        success: function (e) {
          var a = e.data;
          for (var n in a)
            a.hasOwnProperty(n) &&
              ($("#" + t).append($("<option>", { value: n, text: a[n] })),
              o.push(n));
          "" === $("#projectId").val() ||
            isShow ||
            ($("#" + t).val(currentLoggedInUserId),
  ...
  $("#task_table").on("draw.dt", function () {
    ...
    $(document).on("click", ".task-edit-btn", function (e) {
        ...
        success: function (e) {
          if (e.success) {
            ...
            (n.innerHTML = t.title),
              $("#editTagIds").empty(),
              $.each(a, function (e, t) {
                $("#editTagIds").append($("<option>", { value: e, text: t }));
              });
            var i = t.description;
            quillEdit.clipboard.dangerouslyPasteHTML(0, i),
              $("#tagId").val(t.id),
              $("#editTitle").val(n.value),
              $("#taskEditDescription").val(t.description),
              $("#editDueDate").val(t.due_date),
              $("#editProjectId").val(t.project.id).trigger("change"),
              $("#editStatus").val(t.status);
            var o = [],
              s = [];
            (r = []),
              $(t.tags).each(function (e, t) {
                o.push(t.id);
              }),
              $(t.task_assignee).each(function (e, t) {
                s.push(t.id), r.push(t.id);
              }),
              $("#editTagIds").val(o).trigger("change"),
              $("#editAssignee").val(s).trigger("change"),
              $("#editPriority").val(t.priority).trigger("change"),
              setTimeout(function () {
                $.each(t.task_assignee, function (e, t) {
                  $("#editAssignee option[value='" + t.id + "']")
                    .prop("selected", !0)
                    .trigger("change");
                }),
    ...
    $(document).on("click", ".task-delete-btn", function (e) {
      var t = $(e.currentTarget).attr("data-id");
      deleteItem(route("tasks.destroy", t), "#task_table", "Task");
    }),
    $(document).on(
      "click",
      ".delete-recent-task,.delete-older-task",
      function (e) {
        var t = $("#search").val(),
          a = $(e.currentTarget).attr("data-id"),
          n = localStorage.getItem("currentTaskId");
        if (a == n) return displayErrorMessage("Please stop task timer."), !1;
        swal(
          {
            title: deleteHeading + " !",
          ...
          function () {
            $.ajax({
              url: route("tasks.destroy", a),
              type: "DELETE",
              dataType: "json",
              success: function (e) {
                e.success &&
                  (window.livewire.emit("refresh"), $("#search").val(t)),
                  swal({
                    title: "Deleted!",
    ...
    $(document).on("submit", "#addNewForm", function (e) {
      ...
      var t =
          ...
          $("<div />")
            ...
            .trim()
            .replace(/ \r\n\t/g, ""),
        a = jQuery(this).find("#btnTaskSave");
      if ((a.button("loading"), $("#taskDescription").summernote("isEmpty")))
        $("#taskDescription").val("");
      else if (t)
        return (
          displayErrorMessage(
    ...
    $(document).on("submit", ".editForm", function (e) {
      var t =
        "" ===
        $("<div />")
          .html($("#taskEditDescriptionContainer").summernote("code"))
          .text()
          .trim()
          .replace(/ \r\n\t/g, "");
      $(this);
      e.preventDefault();
      var a = jQuery(this).find('[type="submit"]');
      a.button("loading");
      var n = $(this).find(".taskId").val(),
        i = getItemFromLocalStorage("task_id"),
        r = getItemFromLocalStorage("clockRunning");
      if (n === i && "true" === r)
        return (
          s.ajax.reload(),
          swal({
    ...
    $(function () {
      $(document).on("change", "input.complete-task-checkbox", function (a) {
        var n = $(this).attr("data-check");
        t = $(this);
        var i = getItemFromLocalStorage("task_id"),
          r = getItemFromLocalStorage("clockRunning");
        if (n === i && "true" === r)
          return (
            s.ajax.reload(),
        ...
        "" != $("#filter_status").val() &&
          ...
          "undefined" != typeof isTask && e(n);
      }),
        (e = function (e) {
          $.ajax({
            url: route("task.update-status", e),
            type: "POST",
            cache: !1,
            success: function (e) {
              e.success && (window.livewire.emit("refresh"), revokerTracker());
            },
          });
        });
    }),
    (window.manageCollapseIcon = function (e) {
      "true" === $("#tdCollapse" + e).attr("aria-expanded")
        ? ($("#tdCollapse" + e)
            .find("a span")
    ...
    (window.deleteTimeEntry = function (e) {
      swal(
        {
          title: "Delete !",
        ...
        function () {
          $.ajax({
            url: route("time-entries.destroy", e),
            type: "DELETE",
            dataType: "json",
            success: function (e) {
              e.success && $(".close").trigger("click"),
                swal({
                  title: "Deleted!",
    ...
    $(document).on("click", ".timeEntryAddModal", function (e) {
      $("#timeEntryAddModal").appendTo("body").modal("show");
      var t = $(e.currentTarget).attr("data-id"),
        a = $(e.currentTarget).attr("data-project-id");
      $("#timeProjectId").val(a).trigger("change"),
        getTasksByProject(a, "#taskId", t, "#tmValidationErrorsBox"),
        setTimeout(function () {
          $("#taskId").val(t).trigger("change");
        }, 1500);
    }),
    ...
    $(document).on("change", "#editProjectId", function (e) {
      d($(this).val(), "editAssignee"),
        setTimeout(function () {
        ...
        : (l($(this)),
          "" == t.trim()
            ...
            : ($(this).removeClass("error"),
              void window.livewire.emit("updateTask", t, e)));
    }),
    $(".modal").modal({ show: !1, backdrop: "static" }),
  ...
  document.addEventListener("livewire:load", function (e) {
    window.livewire.hook("message.processed", function () {
      $("[rel=tooltip]").tooltip({ placement: "top" }),
        $(document).on("click", ".task-name", function () {
          (n = $(this).text()),
            $(this).hide(),
            (a = $(this).prev("input")).show().val(n).focus();
        });
      var e = document.querySelectorAll(".editDueDate");
      "undefined" != typeof isTask &&
        $(e).datetimepicker({
          format: "YYYY-MM-DD",
          useCurrent: !1,
          locale: "ar" == languageName ? "en" : languageName,
        ...
        $(".editPriority").select2({ width: "100%" }),
        canManageTags
          ? $(".editTagIds").select2({
              width: "100%",
              placeholder: "Select Tags",
              ...
              createTag: function (e) {
                ...
                if (
                  ($("#editTagIds option").each(function () {
                    $.trim(e.term).toUpperCase() ===
                      $.trim($(this).text()).toUpperCase() && (a = !0);
                  }),
                  !a)
                )
                  return { id: e.term, text: t.value };
              },
            })
          : $(".editTagIds").select2({
              width: "100%",
      ...
      var t = document.querySelectorAll("div.task-detail"),
        i = 0;
      $.each(t, function (e, t) {
        $(".taskEditDescriptionContainer-" + e).summernote("destroy");
      }),
        $.each(t, function (e, t) {
          c[i++] = $(".taskEditDescriptionContainer-" + e).summernote({
            placeholder: "Add Task description...",
            minHeight: 200,
            toolbar: [
              ["style", ["bold", "italic", "underline", "clear"]],
    ...
    $(document).on("dp.change", ".editDueDate", function (e) {
      var t = $(this).val();
      window.livewire.emit("updateTaskDueDate", t, i);
    }),
    ...
    $(document).on("change", "#filter_user", function () {
      ...
      var e = isEmpty($(this).val()) ? loginUserId : $(this).val();
      $.ajax({
        url: route("project-by-users", e),
        type: "GET",
        success: function (e) {
          if (e.success) {
            var t = e.data;
            for (var a in ($("#project_filter")
              .find("option")
              .remove()
              .end()
    ...
    $(document).on("click", ".edit-task-assignees", function (e) {
      var t = $(e.currentTarget).attr("data-id");
      startLoader(),
        $.ajax({
          url: route("task.edit-assignee", t),
          type: "GET",
          success: function (e) {
            if (e.success) {
              var t = e.data.task,
                a = e.data.users;
              for (var n in ($("#editAssigneeField").empty(), a))
                a.hasOwnProperty(n) &&
                  $("#editAssigneeField").append(
                    $("<option>", { value: n, text: a[n] })
                  );
              $("#hdnTaskId").val(t.id);
              var i = e.data.task_assignee;
              (r = e.data.task_assignee),
                $("#editAssigneeField").val(i).trigger("change"),
                setTimeout(function () {
                  $("#editAssigneeField")
                    .val(e.data.task_assignee)
                    .trigger("change");
                }, 1e3),
                stopLoader(),
    ...
    $(document).on(
      "click",
      "#EditAssigneeModal #btnSaveAssignees",
      function () {
        var e = jQuery(this);
        e.button("loading"),
          window.livewire.emit(
            "updateAssignees",
            $("#editAssigneeField").val(),
            $("#hdnTaskId").val()
          ),
  ...
  var l = function (e) {
      e.closest(".task-item").addClass("loading"),
        setTimeout(function () {
          e.closest(".task-item").removeClass("loading");
        }, 3e3);
    },
    u = null,
    m = null,
    ...
    f = null;
  function h() {
    document.querySelector("#Add_attachment").addEventListener("change", y, !1);
  }
  $(document).on("click", ".editRecord", function () {
    (u = $(this).data("record-index")),
      (m = $(document)
        .find("[data-edit-priority='" + u + "']")
        .val()),
      (p = $(document)
        .find("[data-edit-tags='" + u + "']")
        .val()
        .toString()
        .split(",")),
      (g = $(document)
        .find("[data-edit-project='" + u + "']")
        .val()),
      (f = $(c[u]).summernote("code"));
  }),
    $(document).on("click", ".elementCancel", function () {
      $(document)
        ...
        $(document)
          ...
          .val(g)
          .trigger("change"),
        $(c[u]).summernote("code", f);
    }),
    $(".task-action .dropdown").hover(
      function () {
    ...
    $(document).on("click", "#resetFilters", function () {
      $("#project_filter").val(null).trigger("change"),
        ...
        window.livewire.emit(
          ["filterTasksByProject", $("#project_filter").val()],
  ...
  var v = [],
    ...
    k = {};
  function y(e) {
    e.preventDefault(), $("#notFoundYet").text(""), (k = e.target.files);
    for (var t = "", a = 0; a < k.length; a++) {
      var n = k[a].name.split(".").pop().toLowerCase();
      if (
        -1 ==
        $.inArray(n, [
          "png",
      ...
      var i = URL.createObjectURL(e.target.files[a]);
      ("xlsx" !== n && "xls" !== n && "csv" !== n) ||
        (i = "/assets/img/xls_icon.png"),
        "pdf" === n && (i = "/assets/img/pdf_icon.png"),
        ("docx" !== n && "doc" !== n) || (i = "/assets/img/doc_icon.png"),
        (t +=
          '\n                <div class="col-sm-3 mt-3 text-center" id="divImageId'
            .concat(
              a,
              '">\n           \n                                    <a class="fancybox preview-image-thumb" target="_blank"\n                                                                   alt="#"> <img class="x-image"  src="'
            )
            .concat(
              i,
              '" />\n                                                               </a>\n                                                     \n                 <div class="x-details">\n                      \n                                                                <div class="x-actions"><strong>\n                                                                        <a href="" class="download-attachment"><?php echo __(\'messages.expense.download\')?> <span class="x-icons"><i class="ti-download"></i></span></a></strong>\n                                                                    <span>\n                                                             \n                                                                    <a href="javascript:void(0)"\n                                                                                class="text-danger delete-attachment" data-id=""><?php echo __(\'messages.common.delete\') ?> </a> </span>\n                                                                                <a href="javascript:void(0)" class="text-danger delete-add-attachment text-center text-decoration-none "   data-key="'
            )
            .concat(a, '" data-id="divImageId')
            .concat(
              a,
              '">Delete</a>\n                                                               \n                                                                </div>\n                                                            </div>\n                 </div>'
            ));
  ...
  function x(e) {
    ...
    for (var n = 0; n < t.length; n++) {
      var i = t[n].name.split(".").pop().toLowerCase();
      if (
        -1 ==
        $.inArray(i, [
          "png",
      ...
      r.append("file", t[n]),
        $.ajax({
          url: route("task.add-attachment", a),
          type: "post",
          data: r,
          cache: !1,
          contentType: !1,
                ...
                (a = [
                  {
                    ...
                    updated_at: moment(t.updated_at).fromNow(),
                    createdId: t.userId,
                    loginUserId: loggedInUserId,
                  },
                ]),
                $("#previewImageEdit").append(
                  prepareTemplateRender("#attachmentImage", a)
                );
            }
  ...
  $(document).on("click", ".delete-add-attachment", function (e) {
    var t = $(e.currentTarget).attr("data-id"),
      a = $(e.currentTarget).attr("data-key");
    swal(
      {
        title: deleteHeading + " !",
      ...
      function () {
        ...
        v.splice(e, 1),
          ...
          w.items.remove(a),
          (document.getElementById("Add_attachment").files = w.files),
          $("#" + t).remove(),
          0 == v.length &&
            $("#notFoundYet")
              .append("No attachments added yet")
    ...
    document.addEventListener(
      ...
      function () {
        1 != isShowProject &&
          document
            .querySelector("#editTaskAddAttachment")
            .addEventListener("change", x, !1);
      },
      !1
    ),
    $(".btn-upload").hide(),
    (window.scrollToTheBottomAttachment = function () {
      setTimeout(function () {
        var e = $("#card-attachments-container-edit").outerHeight();
        $(".attachments-content-edit").scrollTop(e * e);
      }, 200);
    }),
    $(document).on("click", ".delete-attachment", function () {
      var e = $(this).attr("data-id"),
        t = $("#taskId").val(),
        a = $(this).parent().parent().parent().parent().prop("id");
      swal(
        {
          title: deleteHeading + " !",
          text: deleteMessage + ' "' + deleteAttachment + '" ?',
        ...
        function () {
          $.ajax({
            ...
            type: "post",
            url: route("task.delete-attachment", e),
            data: { filename: name, id: t },
            success: function () {
              swal({
                title: "Deleted!",
                text: "Attachment has been deleted.",
