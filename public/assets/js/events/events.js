(() => {
  ...
  $(document).ready(function () {
    $("#AddModal,#EditModal").on("show.bs.modal", function () {
      ...
      $("#type,#editType").select2({ width: "100%" }),
    ...
    $("#calenders").fullCalendar({
      ...
      eventDrop: function (e) {
        permission && eventsUpdateForCalendar(e);
      },
      eventClick: function (e, t) {
        if (permission) {
          var n = t.target.classList.value;
          "fa fa-trash" == n || "action-div justify-content-around" == n
            ? deleteEvent(e)
            : editEvent(e);
        }
      },
      eventRender: function (e, t) {
        t.find(".fc-time").empty(),
          permission &&
            t
              .find(".fc-content")
              .append(
          ...
          2 != e.type &&
            t
              .find(".fc-title")
              .append("<br>" + e.start_time + " to " + e.end_time);
      },
      eventAfterRender: function (e, t) {
        permission || t.find(".fc-content").css("cursor", "default"),
          isEmpty(e.description) ||
            $(t).tooltip({ title: e.description, html: !0, container: "body" });
      },
    });
    var e = $("#eventsCalendarFilter").val();
    $(document).ready(function () {
      getEventsDataForCalendar(e);
    }),
      $(".fc-today-button").click(function () {
        $("#eventsCalendarFilter").val(moment().format("MMMM/Y")),
          (e = $("#eventsCalendarFilter").val());
      }),
      $(".fc-agendaWeek-button").click(function () {
        e == moment().format("MMMM/Y") &&
          $("#calenders").fullCalendar(
            "changeView",
            "agendaWeek",
      ...
      $(document).on("dp.change", "#eventsCalendarFilter", function () {
        (e = $(this).val()), startLoader(), getEventsDataForCalendar(e);
      }),
      (window.getEventsDataForCalendar = function (e) {
        $.ajax({
          ...
          success: function (t) {
            screenUnLock(),
              t.success &&
                ...
                $("#calenders").fullCalendar("addEventSource", t.data),
      ...
      (window.eventsUpdateForCalendar = function (e) {
        var t = e.id,
          n = moment(e.start).format("YYYY-MM-DD HH:mm:ss"),
          a = moment(e.end).format("YYYY-MM-DD HH:mm:ss");
        $.ajax({
          url: route("events.drop.update", t),
          type: "POST",
          data: { start_date: n, end_date: a },
          success: function (e) {
            e.success && getEventsDataForCalendar();
          },
          error: function (e) {
            manageAjaxErrors(e);
          },
          ...
          $.ajax({
            ...
            success: function (e) {
              ...
              e.success &&
                ...
                $("#editStartDate").val(
                  moment(t.start_date).format("YYYY-MM-DD HH:mm")
                ),
                $("#editEndDate").val(
                  moment(t.end_date).format("YYYY-MM-DD HH:mm")
                ),
                $("#editEndDate").data("DateTimePicker").minDate(t.start_date),
                $("#editType").val(t.type).trigger("change"),
                $("#editDescription").summernote("code", t.description),
                $("#Id").val(t.id),
      ...
      $(document).on("submit", "#addNewForm", function (t) {
        ...
        var n =
            ...
            $("<div />")
              ...
              .trim()
              .replace(/ \r\n\t/g, ""),
          a = jQuery(this).find("#btnSave");
        if ((a.button("loading"), $("#description").summernote("isEmpty")))
          $("#description").val("");
        else if (n)
          return (
            displayErrorMessage(
      ...
      $(document).on("submit", "#editForm", function (t) {
        ...
        var n = $("#Id").val(),
          a =
            ...
            $("<div />")
              ...
              .trim()
              .replace(/ \r\n\t/g, ""),
          o = jQuery(this).find("#btnSave");
        if ((o.button("loading"), $("#editDescription").summernote("isEmpty")))
          $("#editDescription").val("");
        else if (a)
          return (
            displayErrorMessage(
        ...
        $.ajax({
          ...
          error: function (e) {
            printErrorMessage("#editValidationErrorsBox", e);
          },
          complete: function () {
            o.button("reset");
          },
      ...
      (window.deleteEvent = function (t) {
        swal(
          ...
          function () {
            $("#calenders").fullCalendar("editable", !1),
              $.ajax({
                url: route("events.delete", t.id),
                type: "DELETE",
                dataType: "json",
                success: function (t) {
                  t.success && getEventsDataForCalendar(e),
                    swal({
                      title: "Deleted!",
