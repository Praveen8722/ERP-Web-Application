(() => {
  ...
  $(window).on("load", function () {
    ...
    (window.loadUsersProjectStatus = function () {
      $.ajax({ type: "GET", url: projectStatusUrl, cache: !1 }).done(
        prepareUsersProjectStatusChart
      );
    }),
    (window.prepareUsersProjectStatusChart = function (t) {
      $("#users-project-status-container").html("");
      var s = t.data;
      if (0 === s.totalRecords.length)
        return (
      ...
      var a = document.getElementById("users-project-status").getContext("2d");
      (a.canvas.style.height = "350px"), (a.canvas.style.width = "100%");
      var n = {
        labels: s.labels,
        datasets: [
          {
      ...
      (window.myBar = new Chart(a, {
        ...
        $("#users-project-status-container").append(
          setStatusTemplate(s.dataPoints, s.labels)
        );
    }),
    (window.setStatusTemplate = function (t, s) {
      return (
