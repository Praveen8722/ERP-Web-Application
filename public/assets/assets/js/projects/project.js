(() => {
  ...
  e.on("change", function () {
    ...
    if (wc_hex_is_light(t))
      return (
        ...
        void $(':input[id="btnSave"]').prop("disabled", !0)
      );
    ...
    t.on("change", function () {
      var e = t.getColor().toHEXA().toString();
      if (wc_hex_is_light(e))
        return (
    ...
    $(document).ready(function () {
      $("#client_id,#edit_client_id").select2({
        ...
        $("#currency,#editCurrency").select2({ width: "100%" }),
        $("#user_ids,#edit_user_ids").select2({
          width: "100%",
          placeholder: "Select Users",
        }),
        $("#editProjectUser").select2({ width: "100%" }),
    ...
    $(document).on("click", ".edit-project-assignees", function (e) {
      ...
      startLoader(),
        $.ajax({
          ...
          success: function (e) {
            if (e.success) {
              var t = e.data.project.id,
                r = e.data.project.allUsers;
              $.each(r, function (e, t) {
                $("#editProjectUser").append(
                  $("<option>", { value: t, text: t.name })
                );
              }),
                $("#hdnProjectId").val(t);
              var i = e.data.users;
              $("#editProjectUser").val(i).trigger("change"),
                $("#editProjectUser").val(e.data.users).trigger("change"),
                stopLoader(),
    ...
    $("#assignProjectUserModal").on("hidden.bs.modal", function () {
      $("#editProjectUser").val(null).trigger("change"),
        jQuery("#btnSaveAssigneesProject").button("reset");
    }),
  ...
  var r = $("#projects_table").DataTable({
    processing: !0,
    serverSide: !0,
    order: [[0, "asc"]],
    ajax: {
      ...
      data: function (e) {
        e.filter_client = $("#filterClient").find("option:selected").val();
      },
    },
    columnDefs: [
      { targets: [0], className: "text-center", width: "7%" },
      { targets: [3], orderable: !1, className: "text-center", width: "5%" },
    ],
    columns: [
      { data: "prefix", name: "prefix" },
      { data: "name", name: "name" },
      { data: "client.name", defaultContent: "", name: "client.name" },
      {
        data: function (e) {
          return (
            '<a title="Edit" class="btn action-btn btn-primary btn-sm edit-btn mr-1" data-id="' +
            e.id +
            '"><i class="cui-pencil action-icon"></i></a><a title="Delete" class="btn action-btn btn-danger btn-sm delete-btn" data-id="' +
            e.id +
  ...
  $(document).on("submit", "#color", function () {
    ...
    $(document).on("submit", "#addNewForm", function (e) {
      ...
      (r[r.length] = { name: "color", value: $("#color").val() }),
        $.ajax({
    ...
    $(document).on("submit", "#editFormProject", function (e) {
      ...
      $(this);
      var r = jQuery(this).find("#btnEditSave");
      if ((r.button("loading"), $("#editDescription").summernote("isEmpty")))
        ...
        $.ajax({
          ...
          success: function (e) {
            if (($("#client_id").empty(), e.success)) {
              ...
              $.each(e.data.clients, function (e, r) {
                (t += "<option value='" + e + "'>" + r + "</option>"),
                  $("#client_id, #edit_client_id").html(t);
              }),
                $("#AddModal").hasClass("show")
                  ? $("#client_id")
    ...
    (window.renderData = function (e) {
      $.ajax({
        url: route("projects.edit", e),
        type: "GET",
        success: function (e) {
          if (e.success) {
            var r = e.data.project,
              i = document.createElement("textarea");
            (i.innerHTML = r.name),
              $("#projectId").val(r.id),
              $("#edit_name").val(i.value),
              $("#edit_prefix").val(r.prefix),
              t.setColor(r.color),
              $("#edit_client_id").val(r.client_id).trigger("change"),
              $("#editStatusProject").val(r.status).trigger("change"),
              $("#editDescription").summernote("code", r.description),
              isEmpty(r.price) ||
                $("#editPrice").val(getFormattedPrice(r.price)),
              $("#editCurrency").val(r.currency).trigger("change"),
              $("#edit_budget_type").val(r.budget_type).trigger("change");
            var a = e.data.users;
            $("#edit_user_ids").val(a),
    ...
    $(document).on("click", ".delete-btn", function (e) {
      var t = $(e.currentTarget).attr("data-id"),
        i =
          ...
          "</div></div>",
        a = getItemFromLocalStorage("project_id"),
    ...
    $("document").ready(function () {
      screenUnLock(), $("#statusOngoing").trigger("click");
    }),
    $(document).on("change", "#projectStatus", function () {
      var e = $(this).val();
      projectStatusLivewire(e);
    }),
    (window.projectStatusLivewire = function (e) {
      window.livewire.emit("projectsStatus", e);
    }),
    $(document).on("change", "#myProjects", function () {
      var e = $(this).data("id");
      1 == $("#myProjects").prop("checked")
        ? window.livewire.emit("usersProject", e)
        : window.livewire.emit("usersProject", null);
    }),
    document.addEventListener("livewire:load", function () {
      window.livewire.hook("message.processed", function () {
        $("#editProjectUser").select2({ width: "100%" }),
          $("#projectStatus").select2({ width: "100%" });
      });
    }),
    $(document).on("focusout", "#name", function (e) {
      var t = $(this).val();
      (t = t.replace(/[^a-z0-9\s]/gi, "").replace(/[_\s]/g, "")),
        $("#prefix").val(t.toUpperCase().slice(0, 8));
    }),
    $(document).on("focusout", "#prefix", function (e) {
      var t = $(this).val();
      (t = t.replace(/[^a-z0-9\s]/gi, "").replace(/[_\s]/g, "")),
        $(this).val(t.toUpperCase().slice(0, 8));
    }),
    $(document).on("focusout", "#edit_name", function (e) {
      var t = $(this).val();
      (t = t.replace(/[^a-z0-9\s]/gi, "").replace(/[_\s]/g, "")),
        $("#edit_prefix").val(t.toUpperCase().slice(0, 8));
    }),
