(() => {
  ...
  $(document).ready(function () {
    $("#taskPriority").select2({ width: "100%" }),
  ...
  $("#taskEstimateTimeHours").datetimepicker({
    ...
    (function (t, a) {
      (e = []),
        ...
        $("#" + a).trigger("change"),
        $.ajax({
          url: route("users-of-projects") + "?projectIds=" + t,
          type: "GET",
          success: function (t) {
            var s = t.data;
            for (var r in s)
              s.hasOwnProperty(r) &&
                ($("#" + a).append($("<option>", { value: r, text: s[r] })),
                e.push(r));
            "" === $("#projectId").val() ||
              isShow ||
              ($("#" + a).val(currentLoggedInUserId),
              $("#" + a).trigger("change.select2"));
          },
        });
    })($("#projectId").val(), "taskAssignee"),
    $(document).on("submit", "#addTaskNewForm", function (e) {
      e.preventDefault();
      var t =
          "" ===
          $("<div />")
      ...
      $.ajax({
        ...
        error: function (e) {
          printErrorMessage("#projectTaskValidationErrorsBox", e);
        },
        complete: function () {
          a.button("reset");
        },
