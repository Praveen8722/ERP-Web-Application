(() => {
  "use strict";
  $(window).on("load", function () {
    loadClientInvoices();
  }),
    (window.loadClientInvoices = function () {
      $.ajax({ type: "GET", url: clientInvoiceUrl, cache: !1 }).done(
        prepareClientInvoiceChart
      );
    }),
    (window.prepareClientInvoiceChart = function (n) {
      $("#client-invoices-container").html("");
      var e = n.data;
      if (
        e.dataPoints.every(function (n) {
          return 0 === n;
        })
      )
        return (
          $("#client-invoices-container").empty(),
          $("#client-invoices-container").append(
            '<div align="center" class="no-record">' +
              noRecordFoundMessage +
              "</div>"
          ),
          !0
        );
      $("#client-invoices-container").html(""),
        $("#client-invoices-container").append(
          '<canvas id="client-invoices"></canvas>'
        );
      var t = document.getElementById("client-invoices").getContext("2d");
      (t.canvas.style.height = "350px"), (t.canvas.style.width = "100%");
      var a = {
        labels: e.labels,
        datasets: [
          {
            data: e.dataPoints,
            backgroundColor: ["#5e001f", "#00e1d9", "#f2bc94"],
          },
        ],
      };
      (window.myBar = new Chart(t, {
        type: "doughnut",
        data: a,
        options: { legend: { display: !1 } },
      })),
        $("#client-invoices-container").append(
          setInvoiceTemplate(e.dataPoints, e.labels)
        );
    }),
    (window.setInvoiceTemplate = function (n, e) {
      return (
        '<div class="row mt-3 py-2 justify-content-center">\n       <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12 text-center">\n           <i class="fas fa-columns mt-3 invoiceStatusDraft"></i>\n           <h3 class="font-weight-normal">\n               <span>' +
        n[0] +
        ' %</span>\n           </h3>\n           <p class="text-muted mb-0">' +
        e[0] +
        '</p>\n       </div>\n       <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12 text-center">\n           <i class="fas fa-columns mt-3 invoiceStatusSent"></i>\n           <h3 class="font-weight-normal">\n               <span>' +
        n[1] +
        ' %</span>\n           </h3>\n           <p class="text-muted mb-0">' +
        e[1] +
        '</p>\n       </div>\n       <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12 text-center">\n           <i class="fas fa-columns mt-3 invoiceStatusPaid"></i>\n           <h3 class="font-weight-normal">\n               <span>' +
        n[2] +
        ' %</span>\n           </h3>\n           <p class="text-muted mb-0">' +
        e[2] +
        "</p>\n  </div>"
      );
    });
})();
