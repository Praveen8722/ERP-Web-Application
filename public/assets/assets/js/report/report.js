(() => {
  ...
  $(document).ready(function () {
    $("#clientId").select2({ width: "100%" }),
      e.select2({ width: "100%" }),
      $("#projectIds").select2({ width: "100%", placeholder: "All Projects" }),
      $("#userIds").select2({ width: "100%", placeholder: "All Users" }),
      $("#tagIds").select2({ width: "100%", placeholder: "Select Tags" }),
    ...
    e.on("change", function () {
      var e, t;
      0 != +$(this).val() && $("#clientId").val(null).trigger("change"),
        (e = 0 === (e = parseInt($(this).val())) ? "" : e),
        $("#clientId").val(null).trigger("change"),
        $("#clientId").empty(),
        $.ajax({
          url: route("clients-of-department") + "?department_id=" + e,
          type: "GET",
          success: function (e) {
            var a = e.data,
              n = '<option value="">All Clients</option>';
            $.each(a, function (e, a) {
              void 0 === t && (t = a),
                (n += '<option value="' + a + '">' + e + "</option>");
            }),
              $("#clientId").html(n),
              $("#clientId").select2(),
              void 0 !== t && $("#clientId").val(t).trigger("change");
          },
        });
    }),
    $(document).on("change", "#clientId", function () {
      var e;
      $("#projectIds").empty(),
        0 != $(this).val() && $("#projectIds").val(null).trigger("change"),
        "" != $(this).val() &&
          ((e = 0 == (e = $(this).val()) ? "" : e),
          $.ajax({
            url: route("projects-of-client") + "?client_id=" + e,
            type: "GET",
            success: function (e) {
    ...
    $("#projectIds").on("change", function () {
      ...
      $("#userIds").empty(),
        ...
        (e = $(this).val().toString()),
        $.ajax({
          url: route("reports.users-of-projects") + "?projectIds=" + e,
          type: "GET",
          success: function (e) {
            var t = e.data;
            for (var a in t)
              t.hasOwnProperty(a) &&
                $("#userIds").append($("<option>", { value: a, text: t[a] }));
          },
        });
    }),
    $(document).on("click", ".delete-btn", function (e) {
      var t = $(e.currentTarget).attr("data-id");
      deleteReport(route("reports.destroy", t));
    }),
    (window.deleteReport = function (e) {
      swal(
        {
          title: deleteHeading + " !",
        ...
        function () {
          $.ajax({
            ...
            success: function (e) {
              e.success &&
                (1 == $("#report_table").DataTable().data().count()
                  ? $("#report_table").DataTable().page("previous").draw("page")
                  : $("#report_table").DataTable().ajax.reload(null, !1)),
                swal({
                  title: "Deleted!",
  ...
  var t = $("#report_table").DataTable({
    ...
    ajax: {
      ...
      data: function (e) {
        e.filter_created_by = $("#filterCreatedBy")
          .find("option:selected")
          .val();
      },
    },
    columnDefs: [
      { targets: [1, 2, 3], width: "10%", className: "text-center" },
      { targets: [4], orderable: !1, className: "text-center", width: "80px" },
    ],
    columns: [
      {
        data: function (e) {
          var t = document.createElement("textarea");
          return (t.innerHTML = e.name), t.value;
        },
        name: "name",
      },
      {
        data: function (e) {
          return format(e.start_date, "YYYY-MMM-DD");
        },
        name: "start_date",
      },
      {
        data: function (e) {
          return format(e.end_date, "YYYY-MMM-DD");
        },
        name: "end_date",
      },
      {
        data: function (e) {
          var t = document.createElement("textarea");
          return (t.innerHTML = isEmpty(e.user) ? "N/A" : e.user.name), t.value;
        },
        name: "owner_id",
      },
      {
  ...
  $(document).on("click", ".preview-btn", function () {
    var e = $(this).closest("form"),
      t = jQuery(this);
    t.button("loading"), $(".enable_preview").prop("checked", !0);
    ...
    (n = n.slice(1)),
      $.ajax({
        ...
        data: n,
        success: function (e) {
          $("section.section").append(e.data), a();
        },
        error: function (e) {
