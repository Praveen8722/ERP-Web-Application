...
const Admincard = ({ organisation, colors, index, handleToggleStatus, handleEdit, handleDelete, activeDropdown, setActiveDropdown }) => {
    const dropdownRef = useRef(null);
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);

  const toggleDropdown = (organisationId, ref) => {
      ...
      <Box className="card-body d-flex align-items-center">
        <div className="author-box-left pl-0 mb-auto mr-3">
          <img
            alt="avatar"
            width="50"
            src={`https://ui-avatars.com/api/?name=${organisation.organisationName}&size=64&rounded=true&color=fff&background=ffaa2e`}
            ...
            <FormControlLabel
              control={
                <Switch
                  ...
                  onChange={() => handleToggleStatus(organisation._id, organisation.isActive)}
                  name="statusSwitch"
                  color={organisation.isActive ? 'primary' : 'error'}
                />
              }
              label={organisation.isActive ? '' : ''}
            />
          </div>
        </div>
        <div className="ml-2 w-100 mb-auto">
          <div className="justify-content-between d-flex">
            <div className="user-card-name pb-1">
              <a href={`https://infyprojects.infyom.com/users/${organisation._id}`}>
                <h4>{organisation.organisationName}</h4>
              </a>
            </div>
            <div className="dropdown" ref={dropdownRef}>
              <FaEllipsisV onClick={() => toggleDropdown(organisation._id, dropdownRef.current)} />
              {isDropdownOpen && activeDropdown === dropdownRef.current && (
                <div className="dropdown-menu show">
                  <button className="dropdown-item" onClick={() => handleEdit(organisation._id)}>
                    <FaEdit className="mr-2" style={{ color: 'yellow' }} /> Edit
                  </button>
                  <button className="dropdown-item" onClick={() => handleDelete(organisation._id)}>
